...
export default function EditDebtModal({ debt, onClose, onSaved }: EditDebtModalProps) {
  const [type, setType] = useState<"borrow" | "lend">(debt.type);
  const [amount, setAmount] = useState(String(debt.amount));
  const [person, setPerson] = useState(debt.person);
  const [description, setDescription] = useState(debt.description);
  const [date, setDate] = useState(debt.date.split("T")[0]);
  const [note, setNote] = useState(debt.note || "");
  const [status, setStatus] = useState<"unpaid" | "paid">(debt.status);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-800 tracking-tight">แก้ไขหนี้</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={person} onChange={e => setPerson(e.target.value)} placeholder="ชื่อเพื่อน/คนที่ยืม/ให้ยืม"
              className="w-full border border-gray-200/60 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all" required />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="บันทึกเพิ่มเติม..."
          ...
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-1.5">สถานะ</label>
            <div className="bg-gray-50 rounded-xl p-1 flex gap-1">
              <button type="button" onClick={() => setStatus("unpaid")}
                className={cn("flex-1 py-2.5 rounded-lg text-sm font-medium transition-all duration-200",
                  status === "unpaid" ? "bg-amber-400 text-white shadow-sm" : "text-gray-400 hover:text-gray-600")}
              >ยังไม่คืน</button>
              <button type="button" onClick={() => setStatus("paid")}
                className={cn("flex-1 py-2.5 rounded-lg text-sm font-medium transition-all duration-200",
                  status === "paid" ? "bg-emerald-500 text-white shadow-sm" : "text-gray-400 hover:text-gray-600")}
              >คืนแล้ว</button>
            </div>
          </div>
